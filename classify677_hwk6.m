function [confusionMatrix,accuracy] =  classify677_hwk6(X,y,options)
%function [confusionMatrix,accuracy] =  classify677_hwk6(Xdata,y,options)
% Input:
    ...
    switch method
        ...
        case 'KNN'
         ...
         case 'SVM'
             ...
             [TrainXdataNorm, mu, sigma] = featureNormalizeSVM(TrainXdata, 0, 0);
             eval(['model = svmtrain(TrainGT,TrainXdataNorm,''-t  ' num2str(svm_t)  ' -c ' num2str(svm_C) ' -g ' num2str(svm_g) ''' );']);
             
             [TestXdataNorm, mu, sigma] = featureNormalizeSVM(TestXdata,mu, sigma);
